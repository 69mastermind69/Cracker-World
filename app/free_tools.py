...
def alphabetical_words(text):
    return "\n".join(
        f"{i}. {word}"
        for i, word in enumerate(sorted(words(text), key=str.lower), 1)
    )
...
def palindrome(text):
    ...
    return "✅ Palindrome" if value == value[::-1] else "❌ Not a palindrome"


def isogram(text):
    value = re.sub(r"[^a-zA-Z]", "", text).lower()
    return "✅ Isogram" if len(value) == len(set(value)) else "❌ Not an isogram"
...
def sort_unique_lines(text):
    return "\n".join(sorted(set(text.splitlines()), key=str.lower))
...
def digit_sum(text):
    ...
    return str(sum(int(x) for x in value)) if value else "0"
...
def prime_factors(text):
    try:
        ...
        return " × ".join(map(str, result)) if result else "No factors."
    except ValueError:
...
def mean2(text):
    try:
        ...
        return str(clean_float(statistics.mean(nums)))
    except ValueError:
...
def median2(text):
    try:
        nums = parse_numbers(text)
        return str(clean_float(statistics.median(nums)))
    except ValueError:
        return "Enter numbers separated by spaces."
...
def variance(text):
    try:
        ...
        return str(clean_float(statistics.variance(nums)))
    except ValueError:
        return "Enter numbers separated by spaces."
...
def stdev(text):
    try:
        ...
        if len(nums) < 2:
            return "Need at least 2 numbers."
        return str(clean_float(statistics.stdev(nums)))
    except ValueError:
        return "Enter numbers separated by spaces."
...
def add_numbers(text):
    try:
        return str(clean_float(sum(parse_numbers(text))))
    except ValueError:
        return "Enter numbers separated by spaces."
...
def dice_many(text):
    try:
        ...
        return " ".join(str(random.randint(1, sides)) for _ in range(count))
    except ValueError:
...
def base32_decode2(text):
    try:
        return base64.b32decode(text.strip()).decode()
    except Exception:
...
def base16_decode2(text):
    try:
        return base64.b16decode(text.strip()).decode()
    except Exception:
...
def csv_columns(text):
    try:
        ...
        first = next(reader, [])
        return "\n".join(first) if first else "No columns."
    except Exception:
        return "Invalid CSV."
...
def leap_year(text):
    try:
        year = int(text.strip())
        return "✅ Leap year" if calendar.isleap(year) else "❌ Not a leap year"
    except ValueError:
...
def add_hours(text):
    try:
        date_text, hours = text.rsplit(" ", 1)
        dt = datetime.strptime(date_text, "%Y-%m-%d %H:%M:%S")
        return (dt + timedelta(hours=float(hours))).strftime("%Y-%m-%d %H:%M:%S")
    except ValueError:
...
def add_minutes(text):
    try:
        date_text, minutes = text.rsplit(" ", 1)
        dt = datetime.strptime(date_text, "%Y-%m-%d %H:%M:%S")
        return (dt + timedelta(minutes=float(minutes))).strftime("%Y-%m-%d %H:%M:%S")
    except ValueError:
...
def rgb_to_hsl(text):
    try:
        r, g, b = [int(x) / 255 for x in text.split()]
        import colorsys
